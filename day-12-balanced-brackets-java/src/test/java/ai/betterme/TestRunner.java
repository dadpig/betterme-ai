 ...
 * with {@code test} in {@link BalancedBracketsTest}, reports PASS/FAIL per
 * test, and exits non-zero if anything fails (so it works in CI / scripts).
 *
 * <p>The assertion API ({@code assertEquals}, {@code assertThrows},
 * {@code assertTrue}, {@code assertFalse}) is deliberately named to match
 * JUnit 5 — swapping this harness for real JUnit 5 is nearly mechanical.
 */
public final class TestRunner {
    ...
    public static void main(String[] args) throws Exception {
        Method[] methods = BalancedBracketsTest.class.getDeclaredMethods();
        ...
        int passed = 0;
        int failed = 0;
        BalancedBracketsTest suite = new BalancedBracketsTest();
    ...
    public static void assertThrows(Class<? extends Throwable> expectedType, Runnable action) {
        try {
            ...
            return; // correct exception type — test passes
        }
        throw new AssertionError("expected " + expectedType.getSimpleName()
